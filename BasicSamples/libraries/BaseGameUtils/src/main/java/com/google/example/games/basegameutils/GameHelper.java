...
public class GameHelper implements GoogleApiClient.ConnectionCallbacks,
    ...
    public void onActivityResult(int requestCode, int responseCode,
        ...
        if (intent != null){
            if (intent.hasExtra(Snapshots.EXTRA_SNAPSHOT_METADATA)){
                ...
                //create a net snapshot named with a unique string
    ...
    /*public PendingResult<Snapshots.CommitSnapshotResult> writeSnapshot(Snapshot snapshot, byte[] data, Bitmap coverImage, String desc){
        //set the
...
*/
        ...
        ResultCallback<Snapshots.OpenSnapshotResult> callback = new ResultCallback<Snapshots.OpenSnapshotResult>() {
            ...
            public void onResult(@NonNull Snapshots.OpenSnapshotResult openSnapshotResult) {
                if (openSnapshotResult.getStatus().isSuccess()){
                    Log.d("LOAD GAME", "SUSSUCEFULL");
                    byte[] data = new byte[0];
                    try{
                        data = openSnapshotResult.getSnapshot().getSnapshotContents().readFully();
                    } catch (IOException e){
                        Log.d("Error reading", "Exception reading load data");
                    }
                    
                    Log.d("SNAPSHOT LOAD", "WORKED");
                } else {
                    
                }
                dismissProgressDiialog();
            }
        };
   ...
   public void loadFromSnapshot(){
       ...
       showProgressDialog("Loading save...");
    ...
    private void savedGamesSelect() {
        ...
        mActivity.startActivityForResult(intent, RC_SELECT_SNAPSHOT);
    }

    private void showProgressDialog(String msg) {
        if (mProgressDialog == null){
            mProgressDialog = new ProgressDialog(mActivity.getApplicationContext());
